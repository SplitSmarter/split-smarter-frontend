...
export default function AddExpenseCategoryScreen() {
    ...
    const handleBack = () => {
        if (router.canGoBack()) {
            ...
            router.replace('/(authenticated)/expense/add');
        }
    };
    ...
    const handleSave = async () => {
        ...
        if (!defaultIcon?.id) {
            showAlert("Default icon not found", "error");
            return;
        }
        ...
        try {
            ...
            if (response.data) {
                ...
                // Route back to AddExpense form passing the new category object as a serialized param
                router.navigate({
                    pathname: '/(authenticated)/expense/add',
                    params: { newlyCreatedCategory: JSON.stringify(response.data) }
                });
            } else {
                ...
                <View className="items-center mb-8 mt-4">
                    <View className="relative">
                        ...
                        <Pressable
                            ...
                            onPress={() => console.log("Image picker integration point")}
                        >
