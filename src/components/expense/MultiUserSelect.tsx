...
const MultiUserSplitSelect = ({type, onSave}: MultiUserSplitSelectProps) => {
    ...
    const handlePercentageModeWorkflow = (id: string, computedAmount: number, activeUsers: UserItem[], lockedState: boolean) => {
        if (lockedState) {
            ...
            const newCalculatedTotal = computedAmount + otherUsersSum;
            const next = activeUsers.map(u =>
                getUserItemKey(u.id, u.user_type) === id ? {...u, amount: computedAmount, isLocked: false} : {
                    ...u,
                    isLocked: false
                }
            );
            ...
            if (!isTotalLockedRef.current) {
                ...
                if (currentSelected.length === 1) {
                    next = [
                        {
                            ...currentSelected[0],
                            amount: totalExpense / 2,
                            isLocked: false,
                            shares: currentSelected[0].shares ?? 1
                        },
    ...
    const onToggleLock = (id: string) => {
        ...
        let next = currentSelected.map(u => getUserItemKey(u.id, u.user_type) === id ? {
            ...u,
            isLocked: !u.isLocked
        } : u);
        const currentlyUnlocked = next.filter(u => !u.isLocked);
    ...
    return (
        ...
        >
            <View className="flex-1">
                {/* Upper Horizontal Quick Suggestions Bar */}
                <SuggestionsSection
                    loading={loading}
                ...
                {/* Scrollable Selected Participants Section */}
                ...
                {/* Fixed Summary & Confirmation Dock */}
                <View
                    className={`p-5 border-t ${
                        isDark ? 'bg-bg-primary-lighter border-gray-800' : 'bg-white border-gray-100'
                    }`}
                >
                    <View
                        className={`flex-row justify-between items-center mb-4 px-4 py-3.5 rounded-2xl ${isDark ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
                        <View className="space-y-0.5">
                            <View className="flex-row items-center gap-x-1.5">
                                <AppText variant="body-small"
                                         className="opacity-50 uppercase tracking-wider font-semibold">Total
                                    Expense</AppText>
                            ...
                            <AppText variant="body-small" className="text-emerald-500 font-medium">
                                Splitting with {selectedUsers.length} {selectedUsers.length === 1 ? 'person' : 'people'}
                            </AppText>
                        </View>
                        <View className="items-end">
                            <AppText variant="h2"
                                     className="font-black tracking-tight text-emerald-600 dark:text-emerald-400">
                                ₹{totalExpense.toFixed(2)}
                            </AppText>
                            {splitMode === 'equal' && selectedUsers.length > 0 && (
                                <AppText variant="caption-xs" className="opacity-40 font-medium mt-0.5">
                                    ₹{(totalExpense / selectedUsers.length).toFixed(2)} each
                                </AppText>
                            )}
                        </View>
                    </View>
                    ...
                    <Pressable
                        ...
                        className="h-14 bg-emerald-600 dark:bg-emerald-500 rounded-2xl flex-row items-center justify-center active:opacity-95 shadow-sm"
                    >
                ...
                <View className="flex-1 items-center justify-center bg-black/60 px-6">
                    <View className={`w-full p-6 rounded-3xl ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
                        <View className="items-center mb-4">
                            ...
                            <AppText variant="h3" className="font-bold text-center text-red-500">Invalid Split
                                Allocation</AppText>
                        </View>
