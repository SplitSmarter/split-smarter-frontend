...
interface SelectCategoryBottomSheetProps {
    visible: boolean;
    selectedId?: number;
    onClose: () => void;
    onSelect: (category: ExpenseCategoryResponse) => void;
}

export const SelectExpenseCategoryBottomSheet = ({
                                                     visible,
                                                     onClose,
                                                     onSelect,
                                                     selectedId
    ...
    const fetchCategories = async () => {
        setLoading(true);
        try {
            const response = await GetExpenseCategoriesApi();
            if (response.data) {
                setCategories(response.data);
            }
        } catch (error) {
            console.error("Failed fetching expense categories:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddNewCategoryPress = () => {
        onClose();
        setTimeout(() => {
            router.push('/(authenticated)/expense/categories/add');
        }, 200);
    };
                ...
                >
                    ...
                    <View className="flex-1 px-6 pt-4">
                        ...
                        {loading ? (
                            ...
                            /* Native ScrollView completely matched to layout grids */
                            <ScrollView
...
SelectExpenseCategoryBottomSheet.displayName = 'SelectExpenseCategoryBottomSheet';